...
class CrudProcess{
    ...
    public function setDefaultColumn(){
        ...
        foreach($dataType as $item){
            ...
            if($item['column_name'] == $this->entity->primaryKey){
                continue;
            }
...
//        $this->entity->pagingLinks = '';
    ...
    public function actionSave(){
        ...
        foreach($createFields as $item){
            if($item == $this->entity->primaryKey){continue;}
    ...
    public function getOneRow(){
        ...
        $row = $row->where('t0.'.$this->entity->primaryKey, '=', $this->entity->ids)->first();
    ...
    public function actionUpdate(){
        ...
        //$id = Input::get($this->entity->primaryKey);
        ...
        if($this->entity->allowEdit and $valid){
            ...
            $status = DB::transaction(function() use($updateData, $insertNN, $insertId, $dataType) {
                ...
                DB::table($this->entity->table)->where(array($this->entity->primaryKey => $this->entity->ids))->update($updateData);
